...
# Cache de símbolos activos (ruta absoluta, anclada al módulo; se puede override por ENV)
MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
CACHE_DIR = os.path.join(MODULE_DIR, ".cache")
os.makedirs(CACHE_DIR, exist_ok=True)
SYMBOL_CACHE_FILE = os.getenv(
    "BINGX_SYMBOL_CACHE_FILE",
...
def _load_symbol_cache() -> dict:
    ...
    candidates = [
        SYMBOL_CACHE_FILE,  # ruta nueva (absoluta)
        ...
        try:
            if os.path.exists(path):
                with open(path, "r") as f:
                    data = json.load(f)
                    # Si alguna vez fue lista, normalízalo a dict {sym: ts}
...
def _update_symbol_cache(symbols: list):
    ...
    try:
        ...
        # limpiar expirados
        current_cache = {
            sym: ts for sym, ts in current_cache.items()
...
def fetch_funding_bingx(limit=100, start_time=None, end_time=None, symbol: str | None = None):
    ...
    try:
        ...
        # ---- FIX robusto contra data=None y estructuras anidadas ----
        recs = []
        if isinstance(data, dict):
            ...
            if isinstance(d, list):
                ...
                # Soporta variantes comunes del payload
                recs = d.get("list") or d.get("rows") or d.get("incomes") or []
        # Si nada encaja, asegúrate de que recs sea lista
        if recs is None:
            recs = []
        # --------------------------------------------------------------
        ...
        return []
## si la funcion de arriba funciona bien, borrar esta.
# def fetch_funding_bingx(limit=100, start_time=None, end_time=None, symbol: str | None = None):
#     """Obtiene historial de funding"""
#     try:
#         params = {"incomeType": "FUNDING_FEE", "limit": min(int(limit), 1000)}
