...
def fetch_spot_trades_for_symbol(symbol: str, days_back: int = 30, limit: int = 1000, 
    ...
    try:
        ...
        data = _mexc_spot_request('GET', '/api/v3/myTrades', params=params, private=True)
    ...
    for symbol in symbols:
        ...
        for pair, trades in by_pair.items():
            ...
            while idx < len(trades) and trades[idx].side == 'sell':
                ...
                if debug:
                    print(f"   🔸 Spotsell inicial ignorado: {f.amount} {base} @ {f.price}")
            ...
            for f in trades[idx:]:
                if f.side == 'buy':
                    ...
                    if (inventory_base <= dust and total_qty_in_round >= 1) or (not lot_q and sell_left <= 1e-12):
                        _flush_round()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--symbols', type=str, nargs='*', help='Símbolos a procesar (default: cache universal)')
    parser.add_argument('--debug', action='store_true', help='Logs verbosos')
    args = parser.parse_args()
    
    print(f"\n🎯 MEXC Spot FIFO (Nueva implementación)")
    print(f"{'='*60}")
    print(f"📊 Símbolos: {'Desde cache universal' if not args.symbols else ', '.join(args.symbols)}")
