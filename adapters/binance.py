...
def fetch_positions_binance(off=0):
    ...
    try:
        ...
        for pos in rows:
            ...
            symbol = normalize_symbol(original_symbol)  # Normalizar símbolo
...
def fetch_positions_binance_enriched(days=60, off=0, debug=False):
    ...
    try:
        ...
        for pos in rows:
            ...
            normalized_sym = normalize_symbol(sym)
            qty = float(pos["positionAmt"])
            side = "long" if qty > 0 else "short"
...
def pull_funding_binance(**kwargs) -> List[Dict[str, Any]]:
    """
    Wrapper mejorado para sync_all_funding que maneja correctamente los parámetros.
    """
    now_ms  = int(time.time() * 1000)
    ...
    if debug:
        print(f"[Binance Funding] force_days={force_d}, since={since}, symbol={symbol}")

    # Si viene 'since', usamos fetch_funding_binance_windowed con since_ms
    if since is not None:
        try:
            since_ms = int(since)
            if debug:
                print(f"[Binance Funding] Usando ventana desde: {_fmt_ms(since_ms)}")
            return fetch_funding_binance_windowed(since_ms=since_ms, until_ms=now_ms, symbol=symbol, off=off, debug=debug)
        except Exception as e:
            if debug:
                print(f"[Binance Funding] Error con since={since}: {e}")
            # Fallback a 7 días
            return fetch_funding_binance_windowed(days=7, symbol=symbol, off=off, debug=debug)
    
    # Si viene force_days, usamos ese número de días
    if isinstance(force_d, int) and force_d > 0:
        if debug:
            print(f"[Binance Funding] Forzando {force_d} días")
        return fetch_funding_binance_windowed(days=int(force_d), symbol=symbol, off=off, debug=debug)
    
    # Default: últimos 7 días
    if debug:
        print("[Binance Funding] Usando default: 7 días")
    return fetch_funding_binance_windowed(days=7, symbol=symbol, off=off, debug=debug)
...
def save_binance_closed_positions(db_path="portfolio.db", days=30, debug=False):
    """
    Guarda posiciones cerradas de Binance en SQLite con mejor deduplicación.
    """
    import sqlite3
    ...
    for pos in positions:
        try:
            ...
            if debug and original_symbol != symbol:
                print(f"🔤 Normalizando símbolo: {original_symbol} → {symbol}")
                
            close_ts = int(pos.get("close_time") or 0)
            size     = float(pos["size"])
            open_ts  = int(pos.get("open_time") or 0)

            # MEJOR DEDUPLICACIÓN: Usar exchange + symbol + open_time + close_time + size
            cur.execute("""
                SELECT 1 FROM closed_positions
                WHERE exchange = ? AND symbol = ? AND open_time = ? AND close_time = ? AND ABS(size - ?) < 1e-8
            """, (exchange, symbol, open_ts, close_ts, size))
            
            if cur.fetchone():
                if debug:
                    print(f"↩️  Skip duplicado: {exchange} {symbol} open={open_ts} close={close_ts} size={size}")
                skipped += 1
                continue
            ...
            funding        = float(pos.get("funding_fee", 0.0))
...
# Funcion de debug temporal para el funding

def debug_binance_funding():
    """Función temporal para debuggear funding de Binance"""
    print("=== DEBUG BINANCE FUNDING ===")
    ...
    print(f"uNow: {_fmt_ms(now_ms)}")
    print(f"1 día: {_fmt_ms(one_day_ago)}")
    print(f"2 días: {_fmt_ms(two_days_ago)}")
    
    # Probar la función directamente
    try:
        funding_data = fetch_funding_binance_windowed(days=2, debug=True)
        print(f"✅ Funding data encontrada: {len(funding_data)} registros")
        
        for f in funding_data[:5]:  # Mostrar primeros 5
            print(f"  - {f['symbol']}: {f['income']} at {_fmt_ms(f['timestamp'])}")
