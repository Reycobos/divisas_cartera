...
Dependencias
------------
- adapters/gate.py  → _request(), fetch_gate_spot_balances()
...
def get_existing_trade_hashes(db_path: str) -> set:
    """
    Obtiene un set de hashes únicos para trades ya existentes en la base de datos.
    El hash se basa en exchange + symbol + side + close_time + size
    """
    ...
    for pair, trades in by_pair.items():
        ...
        for f in trades[idx:]:
            if f.side == "buy":
                ...
                # cantidad REAL recibida en base (si fee es en BASE, descuéntala)
        ...
        if lot_q:
            ...
            if sells_occurred and rem_base <= dust:
                # prácticamente cerrada (solo polvo) → ciérrala
                _flush_round()
            else:
