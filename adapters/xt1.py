...
# =========================================================
#         NUEVO: FUNCIÓN PARA OBTENER PRECIOS
# =========================================================

def _get_spot_prices(cli_s: Spot) -> Dict[str, float]:
    """
    Obtiene los precios actuales de todos los pares vs USDT.
    Retorna un diccionario: {currency: price_in_usdt}
    
    Usa el endpoint público /v4/public/ticker/24h para obtener los precios.
    """
    prices = {}
    
    # Stablecoins se cuentan como 1 USDT
    ...
    try:
        ...
        res = cli_s.req_get('/v4/public/ticker/24h', params=None)
        ...
        for ticker in tickers:
            ...
            symbol = ticker.get('s', '').lower()  # ej: "btc_usdt"
            price = to_float(ticker.get('c') or 0)  # close price (último precio)
            
            if not symbol or price <= 0:
                continue
            
            # Solo nos interesan los pares vs USDT
            if '_usdt' in symbol:
                currency = symbol.replace('_usdt', '')
                prices[currency] = price
                
    except Exception as e:
        print(f"⚠️ Error obteniendo precios spot: {e}")
...
def _get_perp() -> Perp:
    ...
    if _perp_cli is None:
        ...
        # 🔇 Silenciar prints verbosos de pyxt._fetch (method/url/headers/params…)
        try:
            import io, contextlib, os
            # permite reactivar el log con variable de entorno
            quiet = os.getenv("XT_HTTP_DEBUG", "0") != "1"
            if quiet and hasattr(_perp_cli, "_fetch"):
                _orig_fetch = _perp_cli._fetch  # método bound del cliente
                def _quiet_fetch(*args, **kwargs):
                    # oculta cualquier print en stdout dentro de _fetch
                    with contextlib.redirect_stdout(io.StringIO()):
                        return _orig_fetch(*args, **kwargs)
                _perp_cli._fetch = _quiet_fetch
        except Exception as e:
            # Si por cualquier motivo no podemos parchear, seguimos sin romper nada
            print(f"⚠️ No pude silenciar pyxt _fetch: {e}")
    return _perp_cli
...
def fetch_xt_all_balances(db_path: str = "portfolio.db") -> Dict[str, Any]:
    ...
    try:
        ...
        # 🔥 PASO 1: Obtener precios actuales de mercado
        prices = _get_spot_prices(cli_s)
        ...
        if isinstance(assets, list):
            for a in assets:
                ...
                currency = (a.get("currency") or "").lower()
                total_units = to_float(a.get("totalAmount") or 0.0)
                
                if total_units <= 0:
                    continue
                
                # 🔥 CORRECCIÓN: Multiplicar unidades por precio
                price = prices.get(currency, 0.0)
                value_usdt = total_units * price
                
                spot_equity += value_usdt
            
    except Exception as e:
        print(f"⚠️ Error calculando balance spot: {e}")
...
def fetch_xt_open_positions() -> List[Dict[str, Any]]:
    ...
    header = cli._create_sign(
        XT_API_KEY, XT_API_SECRET, path=path,
        bodymod="application/x-www-form-urlencoded", params=params
    )
    header["Content-Type"] = "application/x-www-form-urlencoded"

    url = cli.host + path
    code, success, error = cli._fetch(
        method="GET", url=url, headers=header, params=params, timeout=cli.timeout
    )
    ...
    def _calc_unreal(entry: float, mark: float, size: float, side_txt: str) -> float:
        if size <= 0 or entry <= 0 or mark <= 0:
            return 0.0
        if (side_txt or "").lower() == "short":
            return (entry - mark) * size
        return (mark - entry) * size

    def _estimate_fees_and_funding(position_data: dict) -> Tuple[float, float]:
        """
        Estima fees y funding basado en:
        - Fees: 0.04% del notional como estimación
        - Funding: basado en el tiempo que la posición ha estado abierta
        """
        size = abs(to_float(position_data.get("positionSize") or 0.0))
        entry = to_float(position_data.get("entryPrice") or 0.0)
        mark = to_float(position_data.get("calMarkPrice") or position_data.get("markPrice") or entry)
        
        notional = size * entry
        
        # Estimación de fees (0.04% del notional)
        estimated_fees = -abs(notional * 0.0004)
        
        # Estimación de funding (basada en tamaño y tiempo aproximado)
        # Asumimos que la posición ha estado abierta un promedio de 12 horas
        # con una tasa de funding promedio de 0.01%
        estimated_funding = -(abs(size * mark) * 0.0001 * 0.5)  # 0.5 = 12/24 horas
        
        return estimated_fees, estimated_funding
    ...
    for p in arr or []:
        ...
        side0 = (p.get("positionSide") or "").upper()   # LONG/SHORT
        ...
        liq   = to_float(p.get("breakPrice") or 0.0)
        upnl  = to_float(p.get("floatingPL") or p.get("unrealizedPnl") or 0.0)
        rpnl  = to_float(p.get("realizedProfit") or 0.0)
        margin_isolated = to_float(p.get("isolatedMargin") or 0.0)

        if abs(upnl) < 1e-12:
            upnl = _calc_unreal(entry, mark, size, side)

        # Estimación de fees y funding
        estimated_fees, estimated_funding = _estimate_fees_and_funding(p)
        ...
        out.append({
            ...
            "margin": float(margin_isolated),
            "unrealized_pnl": float(upnl),
            "realized_pnl": float(rpnl),
            "fee": float(estimated_fees),           # Estimado
            "funding_fee": float(estimated_funding), # Estimado
            "leverage": to_float(p.get("leverage") or 1.0),
            "position_type": p.get("positionType") or "UNKNOWN",
        })
...
__all__ = [
    ...
    "debug_dump_xt_position_raw",           # NUEVO
    "debug_dump_xt_position_list_raw",      # NUEVO
]
...
def debug_dump_xt_open_positions_raw(symbol: Optional[str] = None):
    ...
    Útil para verificar floatingPL, realizedProfit, isolatedMargin, etc.
    """
    cli = _get_perp()
    path = "/future/user" + "/v1/position"
    params: Dict[str, Any] = {}
    if symbol:
        params["symbol"] = symbol  # ej: "eth_usdt"
    ...
    url = cli.host + path
    code, payload, error = cli._fetch(
        method="GET", url=url, headers=header, params=params, timeout=cli.timeout
    )

    print(f"HTTP {code} | error={error}")
    print("— RAW —")
    try:
...
#        DEBUG RAW POSITIONS
# ==========================
def debug_dump_xt_position_raw(symbol: Optional[str] = None):
    """
    Imprime el RAW de GET /future/user/v1/position tal cual lo devuelve XT.
    Útil para verificar floatingPL, realizedProfit, isolatedMargin, etc.
    """
    print(f"🔍 DEBUG /future/user/v1/position - symbol: {symbol or 'ALL'}")
    print("=" * 80)
...
def debug_dump_xt_position_list_raw(symbol: Optional[str] = None):
    """
    Imprime el RAW de GET /future/user/v1/position/list tal cual lo devuelve XT.
    """
    print(f"🔍 DEBUG /future/user/v1/position/list - symbol: {symbol or 'ALL'}")
    print("=" * 80)
    ...
    params: Dict[str, Any] = {}
    if symbol:
        params["symbol"] = symbol  # ej: "btc_usdt"
...
if __name__ == "__main__":
    ...
    # NUEVOS DEBUG FLAGS
    parser.add_argument("--position-raw", action="store_true", help="Debug: RAW de /future/user/v1/position")
    parser.add_argument("--position-list-raw", action="store_true", help="Debug: RAW de /future/user/v1/position/list")
    
    parser.add_argument("--symbol", type=str, default=None, help="Símbolo (ej: BTCUSDT o eth_usdt) o None para todos")
    ...
    # NUEVOS DEBUG COMMANDS
    if args.position_raw:
        debug_dump_xt_position_raw(symbol=args.symbol)
    if args.position_list_raw:
        debug_dump_xt_position_list_raw(symbol=args.symbol)

    # DEBUG AUTOMÁTICO AL FINAL (si no se pasó ningún comando)
    if not any([args.balances, args.opens, args.funding, args.transactions, 
                args.closed_preview, args.save_closed, args.position_raw, args.position_list_raw]):
        print("Uso rápido:")
        ...
        print("  python adapters/xt.py --position-raw --symbol btc_usdt   # debug RAW posición individual")
        print("  python adapters/xt.py --position-list-raw                # debug RAW lista de posiciones")
        print("")
        print("🔍 EJECUTANDO DEBUG AUTOMÁTICO DE POSICIONES...")
        print("")
        
        # Debug automático de ambos endpoints
        debug_dump_xt_position_raw()
        print("\n" + "="*100 + "\n")
        debug_dump_xt_position_list_raw()