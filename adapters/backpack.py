...
BACKPACK_OPEN_VERBOSE = os.getenv("BACKPACK_OPEN_VERBOSE", "0") == "1"


def _backpack_debug(msg: str) -> None:
    if BACKPACK_OPEN_VERBOSE:
        print(msg)
...
BACKPACK_API_SECRET = os.getenv(
    "BACKPACK_API_SECRET"
)  # Debe ser la clave privada ED25519 en base64
...
def _bp_sign_message(
    instruction: str, params: dict | None, ts_ms: int, window_ms: int = 5000
) -> str:
    """
    ...
    if query:
        to_sign = (
            f"instruction={instruction}&{query}&timestamp={ts_ms}&window={window_ms}"
        )
    else:
    ...
    if len(seed32) != 32:
        raise RuntimeError(
            f"BACKPACK_API_SECRET must decode to 32 bytes, got {len(seed32)}"
        )
...
def backpack_signed_request(
    ...
    try:
        ...
        print(
            f"[backpack] HTTP error {e.response.status_code} for {path}: {e.response.text}"
        )
        raise
    return r.json()
...
def _normalize_symbol(sym: str) -> str:
    """Normaliza símbolos de Backpack y REMUEVE USDT/USDC del final"""
    if not isinstance(sym, str):
        return sym

    # Remover sufijos como USDT, USDC, PERP, etc.
    s = sym.upper()
    s = re.sub(r"^PERP_", "", s)
    ...
    try:
        ...
        for item in items:
            try:
                ...
                api_unrealized_pnl = float(
                    item.get("pnlUnrealized", 0)
                )  # ← Esto son las FEES
                cumulative_funding = float(item.get("cumulativeFundingPayment", 0))

                # ✅ CALCULAR UNREALIZED PNL MANUALMENTE (mark - entry) * qty
                calculated_unrealized = (mark_price - entry_price) * net_quantity

                # ✅ FEES = pnlUnrealized de la API (Backpack lo reporta así)
                fee_total = api_unrealized_pnl

                # ✅ REALIZED PNL = solo funding (sin fees)
                ...
                positions.append(
                    {
                        ...
                        "unrealized_pnl": calculated_unrealized,  # ← Calculado: (mark-entry)*qty
                        "funding_fee": cumulative_funding,  # ← Funding acumulado
                        "realized_pnl": realized_pnl,  # ← Solo funding
                        "fee": fee_total,  # ← Fees (pnlUnrealized de API)
                        "fee_total": fee_total,  # ← Alias para compatibilidad
                        "notional": notional,
                ...
                if BACKPACK_OPEN_VERBOSE:
                    _backpack_debug(f"[DEBUG] Backpack position: {item.get('symbol')}")
                    _backpack_debug(f"  Quantity: {net_quantity}")
                    _backpack_debug(f"  Entry: {entry_price}, Mark: {mark_price}")
                    _backpack_debug(
                        f"  API pnlUnrealized (=fees): {api_unrealized_pnl}"
                    )
                    _backpack_debug(f"  CALCULATED Unrealized: {calculated_unrealized}")
                    _backpack_debug(f"  Cumulative Funding: {cumulative_funding}")
                    _backpack_debug(f"  Fee Total: {fee_total}")
                    _backpack_debug(f"  Realized PnL (funding only): {realized_pnl}")
                    _backpack_debug(f"  Notional: {notional}")

            except Exception as e:
                print(f"[WARNING] Error processing Backpack position: {e}")
                continue

        _backpack_debug(f"[DEBUG] Backpack positions: {len(positions)} found")
        return positions
        ...
        for item in items:
            try:
                ...
                try:
                    ...
                    if "T" in ts:
                        ts_ms = int(
                            datetime.fromisoformat(ts.replace("Z", "")).timestamp()
                            * 1000
                        )
                    else:
    ...
    for sym, sym_trades in grouped.items():
        ...
        for t in sym_trades:
            ...
            if abs(qty_net) < 1e-9:
                ...
                realized_pnl = sum(
                    (float(tr["price"]) - entry_price)
                    * float(tr["qty"])
                    * (1 if tr["side"].lower() == "sell" else -1)
                    for tr in sym_trades
                    if open_time <= tr["time"] <= close_time
                )
    ...
    try:
        ...
        for fp in funding_payments:
            ...
            if symbol and timestamp:
                ...
                funding_by_symbol_time[symbol][hour_key] = (
                    funding_by_symbol_time[symbol].get(hour_key, 0) + fp["income"]
                )

        if debug:
            print(
                f"💰 Funding payments procesados: {sum(len(v) for v in funding_by_symbol_time.values())} registros"
            )
            ...
            for f in fs:
                ...
                if abs(net) < 1e-9 and len(block) > 1 and max_net_abs > 1e-9:
                    ...
                    if side == "long":
                        ...
                        if entry_trades:
                            entry_avg = (
                                sum(x["qty"] * x["price"] for x in entry_trades)
                                / total_buy
                            )
                        else:
                            entry_avg = 0.0
                        ...
                        if close_trades:
                            close_avg = (
                                sum(abs(x["signed"]) * x["price"] for x in close_trades)
                                / total_sell
                            )
                        else:
                            close_avg = 0.0
                        ...
                        if entry_trades:
                            entry_avg = (
                                sum(abs(x["signed"]) * x["price"] for x in entry_trades)
                                / total_sell
                            )
                        else:
                            entry_avg = 0.0
                        ...
                        if close_trades:
                            close_avg = (
                                sum(x["qty"] * x["price"] for x in close_trades)
                                / total_buy
                            )
                        else:
                            close_avg = 0.0
                    ...
                    if sym in funding_by_symbol_time:
                        for hour_key, funding_amount in funding_by_symbol_time[
                            sym
                        ].items():
                            if open_ms <= hour_key <= close_ms:
                                ...
                                if debug:
                                    print(
                                        f"       Funding: +{funding_amount:.6f} at {datetime.fromtimestamp(hour_key/1000, tz=TZ_ZURICH)}"
                                    )
                    ...
                    results.append(
                        {
                            ...
                            "open_date": datetime.fromtimestamp(
                                open_ms / 1000, tz=TZ_ZURICH
                            ).strftime("%Y-%m-%d %H:%M"),
                            "close_date": datetime.fromtimestamp(
                                close_ms / 1000, tz=TZ_ZURICH
                            ).strftime("%Y-%m-%d %H:%M"),
                        }
                    )
                    ...
                    if debug:
                        ...
                        print(
                            f"     fees={fees:.4f} funding={funding_fee:.4f} pnl={realized_pnl_with_funding:.4f}"
                        )
                        print(f"     net_max={max_net_abs:.2f}, trades={len(block)}")
                        print(
                            f"     PnL breakdown: price={realized_pnl:.4f} + funding={funding_fee:.4f}"
                        )
...
def save_backpack_closed_positions(db_path="portfolio.db", days=60):
    """
    ...
    Args:
        ...
        days: Días hacia atrás para buscar posiciones
    """
    import os
    import sqlite3
    from datetime import datetime
    ...
    closed_positions = fetch_closed_positions_backpack(
        limit=1000, days=days, debug=False
    )
    ...
    try:
        for pos in closed_positions:
            try:
                ...
                # Normalizar el símbolo (agregar USDT al final)
                symbol = _normalize_symbol(original_symbol)
                ...
                funding_in = (
                    pos.get("funding_fee", pos.get("funding_total", 0.0)) or 0.0
                )
                ...
                if realized_in is None:
                    realized_in = (
                        price_pnl_fifo + funding_tot + fee_total_for_db
                    )  # fee_total_for_db ya es negativo
                ...
                if close_ts is None:
                    ...
                    print(
                        f"⚠️ [{symbol}] close_time ausente; se guardará sin chequeo de duplicados."
                    )
                else:
                    cur.execute(
                        """
                ...
                print(
                    f"🔎 [DEBUG closed_backpack_save] {symbol} {side} size={size:.4f}"
                )
                print(f"    entry={entry:.6f} close={close:.6f}")
                print(
                    f"    price_pnl(FIFO)={price_pnl_fifo:.4f}  price_pnl(simple)={simple_pnl:.4f}  Δ={price_pnl_fifo - simple_pnl:.4f}"
                )
                print(
                    f"    fees={fees_abs:.4f}  funding={funding_tot:.4f}  realized(FIFO)={realized_in:.4f}"
                )
                ...
                record = {
                    "exchange": exchange,
                    "symbol": symbol,  # ← Usar símbolo normalizado con USDT
                    "side": side,
                    "size": size,
                    "entry_price": entry,
                    "close_price": close,
    ...
    print(
        f"✅ Guardadas {saved_count} posiciones cerradas de Backpack (omitidas {skipped} duplicadas)."
    )
    ...
    try:
            ...
            for j, t in enumerate(block_trades, start=1):
                ...
                if t["signed"] > 0:  # BUY → cierra shorts primero
                    ...
                    while remaining > EPS and lots and lots[0]["qty"] < 0:
                        ...
                        pnl = (
                            open_lot["price"] - px
                        ) * match_qty  # short: entry - exit
                        pnl_change += pnl
                        realized += pnl
                        open_lot["qty"] += match_qty  # menos negativo
                        remaining -= match_qty
                        matches.append(
                            f"match short {match_qty:.4f} @{open_lot['price']:.4f} -> pnl {pnl:.4f}"
                        )
                        if open_lot["qty"] > -EPS:
                            lots.popleft()
                    if remaining > EPS:
                    ...
                    while remaining > EPS and lots and lots[0]["qty"] > 0:
                        ...
                        remaining -= match_qty
                        matches.append(
                            f"match long {match_qty:.4f} @{open_lot['price']:.4f} -> pnl {pnl:.4f}"
                        )
                        if open_lot["qty"] < EPS:
                            lots.popleft()
                    if remaining > EPS:
                ...
                if abs(net) < EPS:
                    ...
                    pos_str = (
                        f"{'LONG' if net>0 else 'SHORT'} {abs(net):.4f} @ {avg:.4f}"
                    )
        ...
        for symbol, symbol_fills in fills_by_symbol.items():
            ...
            for i, trade in enumerate(symbol_fills):
                print(
                    f"{i+1:2d}. {trade['datetime']} | {trade['side'].upper():4s} | "
            ...
            for i, trade in enumerate(symbol_fills):
                ...
                print(
                    f"  Net antes: {net_before:8.4f} | Net después: {net:8.4f} | Máximo histórico: {max_net_abs:8.4f}"
                )
                ...
                if abs(net) < 1e-9 and len(current_block) > 1 and max_net_abs > 1e-9:
                    ...
                    print(
                        f"   Período: {datetime.fromtimestamp(open_time/1000)} a {datetime.fromtimestamp(close_time/1000)}"
                    )
                    ...
                    side = (
                        "long" if first_trade and first_trade["signed"] > 0 else "short"
                    )
                    ...
                    for j, t in enumerate(current_block):
                        if t["signed"] > 0:  # BUY
                            ...
                            if position_qty == 0:
                                ...
                                position_cost = (
                                    position_cost * position_qty + t["qty"] * t["price"]
                                ) / (position_qty + t["qty"])
                            position_qty += t["qty"]

                            print(
                                f"      {j+1}. BUY  {t['qty']:7.4f} @ {t['price']:8.4f} | "
                                f"Posición: {position_qty:7.4f} @ {position_cost:8.4f}"
                            )

                        else:  # SELL
                            if position_qty > 0:
                                pnl_trade = (t["price"] - position_cost) * abs(
                                    t["signed"]
                                )
                                ...
                                print(
                                    f"      {j+1}. SELL {abs(t['signed']):7.4f} @ {t['price']:8.4f} | "
                    ...
                    if funding_details:
                        for fd in funding_details:
                            print(
                                f"      {fd['time']}: {fd['amount']:9.6f} (rate: {fd['rate']:.6f})"
                            )
                    ...
                    print(
                        f"      {symbol} {side.upper()} | Size: {max_net_abs:.4f} | "
