...
def _realized_funding_last(legs: List[Dict[str, Any]]) -> float:
    """
    WhiteBIT expone 'realizedFunding' a nivel top-level por estado:
    'funding fee for whole position lifetime till current state'.
    Si los legs no traen orderDetail.fundingFee, usamos este acumulado
    del ÚLTIMO estado como fallback.
    """
    for st in reversed(legs):
        if st.get("realizedFunding") is not None:
            return _f(st.get("realizedFunding"))
    return 0.0
def _funding_sum(legs: List[Dict[str, Any]]) -> float:
     return sum(_f(_od(st).get("fundingFee")) for st in legs)

def _funding_total_from_legs(legs: List[Dict[str, Any]]) -> float:
    s_legs = _funding_sum(legs)
    return s_legs if abs(s_legs) > 0 else _realized_funding_last(legs)
# funcion para agregar el funding a la posicion cerrada
# ===== Índice en memoria para funding (por símbolo) =====
from typing import Tuple

def _index_funding_by_symbol(records: List[Dict[str, Any]]) -> Dict[str, List[Tuple[int, float]]]:
    """
    records: [{'symbol': 'BTC', 'timestamp': ms, 'income': float, ...}, ...]
    Devuelve: {'BTC': [(ts_ms, income), ...], ...} ordenado por ts.
    'income' ya sigue tu convención (+ cobro / - pago).
    """
    idx: Dict[str, List[Tuple[int, float]]] = {}
    for r in records or []:
        sym = normalize_symbol((r.get("symbol") or ""))
        ts  = int(_f(r.get("timestamp"), 0.0))
        inc = _f(r.get("income"), 0.0)  # + cobro / - pago
        if not sym or ts <= 0:
            continue
        idx.setdefault(sym, []).append((ts, inc))
...
def save_whitebit_closed_positions(db_path: str = "portfolio.db", days: int = 50, debug: bool = False) -> Tuple[int, int]:
    ...
    grouped = _group_by_position(all_rows)
        # Prefetch de funding en la misma ventana y construir índice en memoria
    try:
        _fund_rows = fetch_whitebit_funding_fees(
            limit=100,            # suficiente para compactar; ajusta si necesitas
            market=None,          # usa todos los mercados detectados por la función
            start_ts=start_ts,    # misma ventana que history
            end_ts=end_ts,
            days_discovery=days
        )
    except Exception as _e:
        print(f"⚠️ WhiteBIT funding prefetch falló: {_e}")
        _fund_rows = []
    _fund_idx = _index_funding_by_symbol(_fund_rows)
    ...
    for pid, legs in grouped.items():
        try:
            ...
            # funding: suma por leg si existe; si no, fallback a realizedFunding del último estado
            funding_total    = _funding_total_from_legs(legs)  # signo natural (API)       # signo natural (API)
            realized_last    = _f(_od(legs[-1]).get("realizedPnl"))

            # precio puro: realized(último) + fees_pos_sum
            price_pnl        = realized_last + fees_pos_sum
            # Fallback: si los legs no trajeron funding, usa la cache de funding en [open..close]
            if abs(funding_total) < 1e-9:
                low  = open_s * 1000  # a ms
                high = close_s * 1000
                lst = _fund_idx.get(sym, [])
                if lst:
                    funding_total = sum(inc for ts, inc in lst if low <= ts <= high)
            
            # Recalcula realized_db tras posible fallback
            ...
            if debug:
                ...
                print(f" fees_sum(+)={fees_pos_sum}  -> fee_total(DB)={fee_total_db}")
                print(f" funding_sum_legs={_funding_sum(legs)} | realizedFunding_last={_realized_funding_last(legs)} | funding_total(escogido)={funding_total}")
...
def debug_whitebit_position_last_trade(position_id: int, days: int = 50):
    ...
    print(f"funding_total={funding_total}")
    print(f"funding_sum_legs={_funding_sum(rows)} | realizedFunding_last={_realized_funding_last(rows)} | funding_total(escogido)={funding_total}")
    print(f"realized_last={realized_last}")
    print(f"pnl(precio)={price_pnl} | realized(DB)={realized_db}")
    print(f"open/close ts: {open_s} / {close_s}")
