...
def _sum_funding_by_symbol(
    ...
    for it in lst:
        ...
        if symbols_filter and sym not in symbols_filter:
            continue
        try:
            # 🔧 CORRECCIÓN: NO invertir signo - la API ya es correcta
            f_val = float(it.get("funding","0"))
            inc = f_val  # Mantener signo original de API
        except:
            inc = 0.0
...
def fetch_bybit_funding_fees(
    ...
    for it in lst:
        ...
        # 🔧 CORRECCIÓN: NO invertir signo - mantener API original
        f_val = _to_float(it.get("funding","0"))
        income = f_val  # Mantener signo API
        ...
        out.append({
            "exchange": "bybit",
            "symbol": base,
            "income": float(income),  # +cobro / -pago (signo API original)
            "asset": currency,
            "timestamp": int(ts),
