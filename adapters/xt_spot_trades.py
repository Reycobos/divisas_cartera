        ...
        if isinstance(result, dict) and "result" in result:
            ...
            if isinstance(result_data, dict) and "items" in result_data:
                ...
                if isinstance(items, list):
                    ...
                    if debug:
                        print(
                            f"  ✅ Procesados {len(items)} trades del endpoint /v4/trade"
                        )
                else:
                    if debug:
                ...
                if debug:
                    print(
                        f"  ⚠️ No se encontró 'items' en result: {list(result_data.keys()) if isinstance(result_data, dict) else type(result_data)}"
                    )
        else:
            if debug:
...
def _insert_row(
    ...
    try:
        ...
        position_data = {
            ...
            "initial_margin": row.get(
                "notional", 0
            ),  # Para spot, initial_margin = notional
...
""",
            ...
            if verbose:
                print(
                    f"💾 [DEBUG] Guardando nueva posición (fallback): {position_hash}"
                )
            # Actualizar el set de hashes existentes
            existing_hashes.add(position_hash)
            return True
            ...
            if verbose:
                print(
                    f"⚠️ [DEBUG] IntegrityError al insertar (ya existe): {position_hash}"
                )
            return False


def save_xt_spot_positions(
    db_path: str = DB_PATH_DEFAULT, days_back: int = 30, debug: bool = False
) -> Tuple[int, int]:
    """
    ...
    if total_trades_found == 0:
        print(f"\n{'='*60}")
        print("ℹ️  No se encontraron trades nuevos en XT Spot")
        print(f"{'='*60}")
        return 0, 0
    ...
    if debug:
        print(f"📊 Procesando {symbols_with_trades} símbolos diferentes")
    ...
    for symbol, trades in by_symbol.items():
        ...
        if base in STABLES and quote in STABLES:
            for f in trades:
                ...
                if _insert_row(
                    conn, row, existing_hashes, verbose=debug
                ):  # Pasar existing_hashes
                    saved += 1
            continue
        ...
        for f in trades[idx:]:
            if f.side == "buy":
                ...
                if (inventory_base <= dust and total_qty_in_round >= 100) or (
...
if __name__ == "__main__":
    ...
    save_xt_spot_positions(db_path=args.db, days_back=args.days_back, debug=args.debug)
