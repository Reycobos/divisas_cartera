...
GATE_VERBOSE_LOGS = os.getenv("GATE_VERBOSE_LOGS", "0") == "1"


def _gate_debug(msg: str) -> None:
    if GATE_VERBOSE_LOGS:
        print(msg)
...
def _sign_v4(method: str, path: str, query: str, body: str, ts: int) -> str:
    ...
    """
    prehash = "\n".join(
        [method.upper(), f"{GATE_PREFIX}{path}", query, _sha512_hex(body), str(ts)]
    )
    return hmac.new(
        GATE_API_SECRET.encode("utf-8"), prehash.encode("utf-8"), hashlib.sha512
    ).hexdigest()


def _headers(
    method: str, path: str, params: Optional[Dict[str, Any]], body_obj: Optional[Any]
) -> Dict[str, str]:
    _require_keys()
    ts = int(time.time())
    ...
    body = (
        ...
        if body_obj is None
            ...
            else json.dumps(body_obj, separators=(",", ":"), ensure_ascii=False)
        )
    )
...
def _contract_to_symbol(contract: str) -> str:
    # "BTC_USDT" -> "BTCUSDT"
    # "BTC/USDT" -> "BTCUSDT" (nuevo formato spot)
...
def fetch_gate_spot_balances() -> List[Dict[str, Any]]:
    ...
    """
    _gate_debug("🔍 DEBUG: Obteniendo balances SPOT de Gate.io...")
    ...
    for row in data or []:
        ...
        if total > 0.01:  # Solo mostrar balances mayores a 0.01
            print(
                f"   💰 SPOT {row.get('currency')}: Available={available:.6f}, Locked={locked:.6f}, Total={total:.6f}"
            )

    _gate_debug(f"📊 DEBUG: Total SPOT balance: {total_spot:.6f} USDT equivalente")
    _gate_debug(f"📦 DEBUG: Se encontraron {len(out)} monedas en spot")
    return out
...
def fetch_gate_margin_balances() -> List[Dict[str, Any]]:
    ...
    """
    _gate_debug("🔍 DEBUG: Obteniendo balances MARGIN de Gate.io...")
    ...
    for row in data or []:
        ...
        risk = row.get("risk")
        _gate_debug(f"   📈 DEBUG MARGIN: Procesando par {cp}, riesgo: {risk}")
        ...
        for leg_key in ("base", "quote"):
            ...
            if abs(net_value) > 0.01 or borrowed > 0.01:
                print(
                    f"      📊 {leg_key.upper()} {leg.get('currency')}: Net={net_value:.6f}, Available={available:.6f}, Borrowed={borrowed:.6f}"
                )

            total_margin_net += net_value

    _gate_debug(f"📊 DEBUG: Total neto MARGIN: {total_margin_net:.6f}")
    _gate_debug(f"📦 DEBUG: Se procesaron {len(out)} legs de margin")
    return out
...
def fetch_gate_futures_balances(settle: str = "usdt") -> Dict[str, Any]:
    ...
    """
    _gate_debug(
        f"🔍 DEBUG: Obteniendo balances FUTURES ({settle.upper()}) de Gate.io..."
    )
    ...
    try:
        ...
        if "USER_NOT_FOUND" in str(e) or "please transfer funds first" in str(e):
            _gate_debug(f"⚠️  DEBUG: No hay cuenta FUTURES {settle.upper()} - {e}")
...
def fetch_gate_open_positions(settle: str = "usdt") -> List[Dict[str, Any]]:
    ...
    """
    _gate_debug(
        f"🔍 DEBUG: Obteniendo POSICIONES ABIERTAS ({settle.upper()}) de Gate.io..."
    )
    settle = (settle or "usdt").lower()
    params = {"holding": "true"}

    data = _request("GET", f"/futures/{settle}/positions", params=params)
    _gate_debug(f"📦 DEBUG: Se recibieron {len(data or [])} posiciones abiertas")
    ...
    if GATE_VERBOSE_LOGS:
        try:
            for i, row in enumerate((data or [])[:3]):
                try:
                    _gate_debug(
                        f"   🧾 RAW[{i}]: {json.dumps(row, ensure_ascii=False)}"
                    )
                except Exception:
                    _gate_debug(f"   🧾 RAW[{i}]: {row}")
        except Exception as e:
            _gate_debug(f"   ⚠️ DEBUG RAW dump error: {e}")
    ...
    for p in data or []:
        ...
        print(
            f"🧪 GATE {symbol} side={side} entry={entry} mark={mark} liq={liquidation_price}"
        )

        out.append(position)

    _gate_debug(f"✅ DEBUG: {len(out)} posiciones abiertas normalizadas")
    return out
...
def fetch_gate_funding_fees(
    ...
    if start_time:
        ...
        print(
            f"   ⏰ Desde: {datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}"
        )
    if end_time:
        params["to"] = int(end_time)
        print(
            f"   ⏰ Hasta: {datetime.fromtimestamp(end_time).strftime('%Y-%m-%d %H:%M:%S')}"
        )
...
def fetch_gate_closed_positions(
    settle: str = "usdt", days_back: int = 30
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        position_history = _request(
            "GET", f"/futures/{settle}/position_close", params=params
        )
        ...
        print(
            f"📦 DEBUG: Se recibieron {len(position_history)} registros de posiciones cerradas"
        )
        ...
        for i, pos in enumerate(position_history):
            ...
            if closed_pos:
                ...
                print(
                    f"      ✅ Normalizada: {closed_pos['symbol']} - PnL: {closed_pos['realized_pnl']}"
                )
            else:
...
def save_gate_closed_positions(
    db_path: str = "portfolio.db", days_back: int = 30
) -> int:
    """
    Guarda posiciones cerradas de Gate.io en SQLite evitando duplicados por (exchange, symbol, close_time).
    Normaliza los símbolos usando la función normalize_symbol.
    """
    print("💾 Guardando posiciones cerradas de Gate.io en portfolio.db")

    # Importar la función de normalización de símbolos
    import sys
    import os

    # Agregar la carpeta utils al path para poder importar symbols
    ...
    for pos in closed_positions:
        try:
            # Normalizar el símbolo (quitar USDT, etc.)
            original_symbol = pos["symbol"]
            normalized_symbol = normalize_symbol(original_symbol)

            print(f"🔤 Normalizando símbolo: {original_symbol} → {normalized_symbol}")

            # Deduplicación por (exchange, symbol, close_time) usando el símbolo normalizado
            cur.execute(
                """
                SELECT COUNT(*) FROM closed_positions
                WHERE exchange = ? AND symbol = ? AND close_time = ?
            """,
                (pos["exchange"], normalized_symbol, pos["close_time"]),
            )
            if cur.fetchone()[0]:
                skipped += 1
                continue
            ...
            position_data = {
                ...
                "symbol": normalized_symbol,  # Usar el símbolo normalizado
                "side": pos["side"],
                "size": pos["size"],
...
def _normalize_gate_closed_position(
    pos_data: Dict, settle: str
    ...
    try:
        ...
        if diff > 0 and abs(pnl_price_only) > 0:
            ...
            if size <= 0 or abs(size_from_pnl - size) / max(1.0, abs(size)) > 0.05:
                print(
                    f"         🔧 Size fix: {size} → {size_from_pnl:.0f} (from price_pnl)"
                )
        ...
        notional = (
            abs(size) * close_price if close_price > 0 else abs(size) * entry_price
        )
        ...
        print(
            f"         ⏰ Open time sec: {open_time_seconds}, Close time sec: {close_time_seconds}"
        )

        # Leverage y liquidation price
        leverage = _num(pos_data.get("leverage"), None)
        liquidation_price = (
            None  # Gate.io no proporciona liquidation_price en position_close
        )
        ...
        print(
            f"         ✅ Posición normalizada: {closed_position['symbol']} - Size: {abs(size)}"
        )
...
def fetch_gate_all_balances(
    settles: Tuple[str, ...] = ("usdt", "btc")
) -> Dict[str, Any]:
    """
