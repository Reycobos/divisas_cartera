...
_KUCOIN_SYM_CACHE = {"ts": 0, "ttl": 300, "symbols": []}  # ttl 5 min

def _sym_cache_set(symbols: list[str]):
    try:
        syms = sorted({(s or "").upper().replace("-", "") for s in symbols if s})
        _KUCOIN_SYM_CACHE["symbols"] = syms
        _KUCOIN_SYM_CACHE["ts"] = int(time.time())
    except Exception:
        pass

def _sym_cache_get() -> list[str]:
    try:
        if int(time.time()) - int(_KUCOIN_SYM_CACHE.get("ts", 0)) <= int(_KUCOIN_SYM_CACHE.get("ttl", 300)):
            return list(_KUCOIN_SYM_CACHE.get("symbols", []))
    except Exception:
        pass
    return []
...
def fetch_kucoin_open_positions() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raw_symbols: list[str] = []               # 👈 Para cache
        ...
        for pos in rows:
            ...
            raw = (pos.get("symbol") or "").upper().replace("-", "")
            if raw:
            ...
            out.append({
                ...
                "symbol_raw": raw,                               # 👈 útil para depurar
                "side": side,
                "size": abs(qty),
                "entry_price": entry,
                "mark_price": mark,
        ...
        # 🧠 cachea los símbolos raw para que funding los reutilice
        if raw_symbols:
            _sym_cache_set(raw_symbols)
...
def _kucoin_symbols_from_db(db_path: str = "portfolio.db", max_rows: int = 200) -> list[str]:
    """
    Lee símbolos preferentemente de la columna 'symbol_raw' (si existe).
    Fallback: usa 'symbol' y mapea a formato KuCoin (BASE + 'USDTM', BTC→XBT).
    """
    try:
        import sqlite3
        conn = sqlite3.connect(db_path)
        cur = conn.cursor()
        cur.execute("PRAGMA table_info(closed_positions)")
        cols = {row[1] for row in cur.fetchall()}

        if "symbol_raw" in cols:
            cur.execute("""
                SELECT DISTINCT symbol_raw FROM closed_positions
                WHERE exchange='kucoin' AND symbol_raw IS NOT NULL AND symbol_raw <> ''
            ...
            cur.execute("""
                ...
                WHERE exchange='kucoin' AND symbol IS NOT NULL AND symbol <> ''
                ORDER BY close_time DESC LIMIT ?
            """, (max_rows,))
            bases = [r[0] for r in cur.fetchall()]
            rows = []
            for b in bases:
                base = (b or "").upper().replace("-", "").replace("_", "")
                # Heurística mínima: BTC→XBT, demás → BASE + USDTM
                if base == "BTC": base = "XBT"
                rows.append(f"{base}USDTM")

        syms = sorted({(s or "").upper().replace("-", "") for s in rows if s})
        return syms
    except Exception as e:
        print(f"⚠️ KuCoin DB symbols error: {e}")
        return []
    finally:
        try: conn.close()
        except: pass

def _kucoin_target_symbols(preferred: Optional[str] = None,
                           db_path: str = "portfolio.db",
                           debug: bool = False) -> list[str]:
    if preferred:
        return [preferred.upper().replace("-", "")]
    syms = _sym_cache_get()
    if debug and syms:
        print(f"KuCoin funding symbols from CACHE: {syms}")
    if syms:
        return syms

    syms = _kucoin_symbols_from_db(db_path=db_path)
    if debug and syms:
        print(f"KuCoin funding symbols from DB: {syms}")
    return syms
    
def fetch_funding_kucoin(limit: int = 5000,
    ...
    """
    GET /api/v1/funding-history  (Futures / Private)
      - Requiere 'symbol' KuCoin (p.ej. XBTUSDTM). Si no se pasa, usamos: CACHE → DB.
      - Ventanas temporales <= 3 meses (recomendado por API).
      - maxCount hasta 1500 por request.
    """
    if not _has_creds():
        return []
    ...
    now_ms = _now_ms()
    if since is None:
        days = 60 if days is None else int(days)
        since = now_ms - days*24*3600*1000
    since = int(since)

    symbols = _kucoin_target_symbols(preferred=symbol, db_path=db_path, debug=debug)
    if debug and not symbols:
        print("⚠️ KuCoin funding: no symbols (ni cache ni DB).")
    if not symbols:
        return []

    # ventana de 85-90 días para cumplir < 3 meses
    step_ms = 85 * 24 * 3600 * 1000
    out: List[Dict[str, Any]] = []

    for sym in symbols:
        start = since
        while start < now_ms and len(out) < limit:
            end = min(start + step_ms, now_ms)
            qs = f"/api/v1/funding-history?symbol={sym}&startAt={start}&endAt={end}&maxCount={min(int(per_req),1500)}&reverse=true"
            try:
                resp = _get(KUCOIN_BASE_URL, qs, "GET")
            except Exception as e:
                print(f"❌ KuCoin funding request error [{sym}] ({start}..{end}): {e}")
                break

            d = resp.get("data", {}) if isinstance(resp, dict) else {}
            items = d.get("dataList", []) or []

            if debug:
                rng = [int(x.get("timePoint", 0) or 0) for x in items]
                print(f"[{sym}] window {start}->{end} count={len(items)} "
                      f"range=({min(rng) if rng else None}..{max(rng) if rng else None})")

            for f in items:
                try:
                    ts = int(f.get("timePoint", 0) or 0)      # 👈 campo correcto
                    if ts and ts < since:
                        continue
                    raw_sym = (f.get("symbol") or "").upper().replace("-", "")
                    out.append({
                        "exchange": "kucoin",
                        "symbol": _normalize_symbol(raw_sym),    # para tu DB/UI
                        "symbol_raw": raw_sym,                   # debug/consistencia
                        "income": _safe_float(f.get("funding", 0)),
                        "asset": f.get("settleCurrency", "USDT"),
                        "timestamp": ts,
                        "funding_rate": _safe_float(f.get("fundingRate", 0)),
                        "type": "FUNDING_FEE",
                        "external_id": str(f.get("id") or f"{raw_sym}|{ts}")
                    })
                    if len(out) >= limit:
                        break
                except Exception as e:
            ...
            start = end
            if not items:
                break  # no hay más en esta ventana

    out.sort(key=lambda x: x["timestamp"] or 0)
    if debug:
        if out:
            print(f"TOTAL KuCoin funding items={len(out)} "
                  f"range=({out[0]['timestamp']}..{out[-1]['timestamp']})")
        else:
            print("TOTAL KuCoin funding items=0")
    return out
# =========================
...
def fetch_closed_positions_kucoin(symbol: Optional[str] = None,
    ...
    try:
        ...
        for pos in items:
            ...
            # Si no encontramos margen, usar notional/leverage como fallback
            ...
            if raw_margin is not None and raw_margin > 0:
                initial_margin_val = raw_margin
            elif notional and lev and lev > 0:
                initial_margin_val = notional / lev
            else:
                initial_margin_val = notional
            ...
            out.append({
                ...
                "initial_margin": float(initial_margin_val) if initial_margin_val is not None else 0.0,
...
def save_kucoin_closed_positions(*args, **kwargs):
    """
    Usos:
      - save_kucoin_closed_positions(position_dict)
      - save_kucoin_closed_positions(db_path="portfolio.db", days=90, debug=False)
    """
    import os, sqlite3
    from db_manager import save_closed_position, DB_PATH as _DEFAULT_DB_PATH
    ...
    def _enrich(pos: dict) -> dict:
        # Normaliza y calcula faltantes para una sola posición (por si el fetch no lo hizo)
        side  = pos.get("side")
        size  = _f(pos.get("size"))
        entry = _f(pos.get("entry_price"))
        close = _f(pos.get("close_price"))
        ft    = _f(pos.get("funding_total", 0.0))
        fe    = -abs(_f(pos.get("fee_total", 0.0)))  # fees siempre negativas
        pnl_p = _f(pos.get("pnl", _price_pnl(side, entry, close, size)))
        realized = _f(pos.get("realized_pnl", pnl_p + ft + fe))

        notional = _f(pos.get("notional", size * entry))
        leverage = _f(pos.get("leverage"))
        ini_m    = pos.get("initial_margin")
        ini_m    = _f(ini_m) if ini_m is not None else None

        if ini_m and ini_m > 0:
            base_cap = ini_m
        elif notional and leverage and leverage > 0:
            base_cap = notional / leverage
        else:
            base_cap = notional

        pnl_pct = pos.get("pnl_percent")
        if pnl_pct is None:
            pnl_pct = (realized / base_cap) * 100.0 if base_cap else 0.0

        open_s  = int(pos.get("open_time") or 0)
        close_s = int(pos.get("close_time") or 0)
        days = max((close_s - open_s) / 86400.0, 1e-9) if (open_s and close_s) else 1e-9
        apr_val = pos.get("apr")
        if apr_val is None:
            apr_val = pnl_pct * (365.0 / days) if days > 0 else 0.0

        # Escribe los campos normalizados de vuelta
        pos = dict(pos)  # copia
        pos["fee_total"]     = fe
        pos["funding_total"] = ft
        pos["pnl"]           = pnl_p
        pos["realized_pnl"]  = realized
        pos["notional"]      = notional
        pos["pnl_percent"]   = pnl_pct
        pos["apr"]           = apr_val
        # asegúrate de guardar initial_margin aunque venga vacío
        pos["initial_margin"] = ini_m if (ini_m and ini_m > 0) else base_cap
        # optional: mantener initial_margin si vino
        return pos

    # -------- dispatcher: single-position vs bulk --------
    if args and isinstance(args[0], dict):
        # Modo: una sola posición
        position = _enrich(args[0])
        save_closed_position(position)
        return

    # Modo: bulk con db_path/days/debug
    db_path = kwargs.get("db_path", "portfolio.db")
    days    = int(kwargs.get("days", 90))
    debug   = bool(kwargs.get("debug", False))

    if not _has_creds():
        print("⚠️ KuCoin: faltan credenciales. No se guardan cerradas.")
        return

    now = _now_ms()
    start = now - days * 24 * 60 * 60 * 1000
    positions = fetch_closed_positions_kucoin(start_time=start, end_time=now, debug=debug)
    print(f"🔍 KuCoin: se obtuvieron {len(positions)} posiciones de la API")
    if not positions:
    ...
    for pos in positions:
        try:
            # dedupe por exchange+symbol+close_time
            cur.execute(
                "SELECT 1 FROM closed_positions WHERE exchange=? AND symbol=? AND close_time=? LIMIT 1",
                (pos.get("exchange"), pos.get("symbol"), pos.get("close_time"))
            )
            if cur.fetchone():
                skipped += 1
                continue

            save_closed_position(_enrich(pos))
            saved += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument("--symbol", type=str, default=None, help="Símbolo KuCoin futures, p.ej. XBTUSDTM")
    ap.add_argument("--days", type=int, default=60)
    ap.add_argument("--since", type=int, default=None)
    ap.add_argument("--limit", type=int, default=5000)
    ap.add_argument("--per-req", type=int, default=1500)
    ap.add_argument("--db-path", type=str, default="portfolio.db")
    args = ap.parse_args()

    if args.funding_debug:
        print("🔎 KuCoin funding DEBUG (cache→DB resolver)")
        rows = fetch_funding_kucoin(limit=args.limit,
                                    symbol=args.symbol,
                                    since=args.since,
                                    days=args.days,
                                    per_req=args.per_req,
        ...
        print(fetch_kucoin_open_positions()[:2])


# no funciona pero tiene los calcullos necesarios
...
# funcione, si la nueva funciona, borrar esta
# def save_kucoin_closed_positions(db_path: str = "portfolio.db",
#                                  days: int = 90,
