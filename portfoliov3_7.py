...
from trades_processingv8 import (
...
def should_sync(exchange_name):
    if SYNC_ALL:
        return True
    return SYNC_EXCHANGES.get(exchange_name, False)


BALANCE_ALL = False  # Si pones True, pedirá balances de TODOS los exchanges
BALANCE_EXCHANGES = {
    "backpack": False,
    "aden": False,
    "bingx": False,
...
KUCOIN_API_SECRET = os.getenv("KUCOIN_SECRET_KEY")
...
#=============codigo que funciona solo con el funding

# def _sum_fees_and_funding_for_block(symbol, trades_block, open_time_ms, now_ms, incomes, debug=False):
#     """
#     Suma fees (COMMISSION) y funding desde open_time_ms hasta now_ms.
#     Commission: emparejada por tradeId preferentemente; si falta, por rango de tiempo.
#     Funding: siempre por timestamp en el rango.
#     """
#     # set de tradeIds del bloque
#     block_ids = {str(int(t["id"])) for t in trades_block if "id" in t}

#     # COMMISSION por tradeId; si no hay, rango de tiempo
#     commissions = [i for i in incomes if i.get("incomeType") == "COMMISSION"]
#     comm_by_tid = [i for i in commissions if (i.get("tradeId") is not None and str(i["tradeId"]) in block_ids)]
#     if not comm_by_tid:
#         comm_by_tid = [i for i in commissions if open_time_ms <= int(i.get("time", 0)) <= now_ms]

#     # FUNDING por rango
#     fundings = [i for i in incomes if i.get("incomeType") == "FUNDING_FEE" and open_time_ms <= int(i.get("time", 0)) <= now_ms]

#     fee_sum = sum(float(i.get("income", 0.0)) for i in comm_by_tid)
#     fnd_sum = sum(float(i.get("income", 0.0)) for i in fundings)

#     if debug:
#         print(f"      fees(items={len(comm_by_tid)}): {fee_sum:.6f} | funding(items={len(fundings)}): {fnd_sum:.6f}")

#     return fee_sum, fnd_sum
        ...
        while True:
            ...
            for row in data:
                try:
                    ...
                    # === PNLs según BingX ===
                    # - realisedProfit: PnL de precio (sin fees/funding) -> "pnl"
                    # - netProfit: PnL neto total (incluye fees y funding) -> "realized_pnl"
                    price_pnl   = float(row.get("realisedProfit") or 0)
                    ...
                    fee_total     = float(row.get("positionCommission") or 0)  # BingX suele devolverlo negativo (coste)
                    lev           = float(row.get("leverage") or 0)
                    side          = (row.get("positionSide") or "").lower()
            
                    # Si la API no trae netProfit, lo recomponemos:
...
def kucoin_server_timestamp_ms() -> str:
    """
    Devuelve el timestamp del servidor KuCoin en milisegundos como string.
    Si falla, cae en hora local (fallback).
    """
    try:
        r = requests.get(f"{KUCOIN_BASE_URL}/api/v1/timestamp", timeout=5)
        r.raise_for_status()
        # El campo 'data' ya viene en ms
...
def fetch_account_kucoin_futures():
    """
    Balance de futuros de KuCoin.
    """
    try:
        endpoint = "/api/v1/account-overview?currency=USDT"
        method = "GET"
        headers = _kucoin_headers(KUCOIN_API_KEY, KUCOIN_API_SECRET, KUCOIN_API_PASSPHRASE,
                                  method, endpoint)
        url = f"{KUCOIN_BASE_URL}{endpoint}"
        r = requests.get(url, headers=headers, timeout=20)
        print("DEBUG kucoin futures account response:", r.status_code, r.text)
        r.raise_for_status()
        data = r.json().get("data", {})
        return {
            "accountequity": float(data.get("accountEquity", 0)),
        ...
        print(f"❌ KuCoin futures account error: {e}")
        return {
            "equity": 0.0,
            "balance": 0.0,
            "unrealized_pnl": 0.0,
...
def fetch_spot_accounts_kucoin():
    """
    Fetch balances de KuCoin spot (cuentas spot). Endpoint: GET /api/v1/accounts
    """
    try:
        ...
        headers = _kucoin_headers(KUCOIN_API_KEY, KUCOIN_API_SECRET, KUCOIN_API_PASSPHRASE,
                                  method, endpoint)
        url = f"{KUCOIN_SPOT_BASE}{endpoint}"
        r = requests.get(url, headers=headers, timeout=20)
        #print("DEBUG kucoin spot account response:", r.status_code, r.text)
        r.raise_for_status()
        arr = r.json().get("data", [])
        # Filtrar cuentas activas o con balance no cero
        balances = []
        for acct in arr:
            bal = float(acct.get("balance", 0))
            avail = float(acct.get("available", 0))
            holds = float(acct.get("holds", 0))
            if bal > 0 or holds > 0:
                balances.append({
        ...
        print(f"❌ KuCoin spot account error: {e}")
        return []

def fetch_all_balances_kucoin():
    """
    Combina saldo de futuros + spot para devolver una vista unificada.
    """
    fut = fetch_account_kucoin_futures()
    spot = fetch_spot_accounts_kucoin()
    total_spot = sum(s["balance"] for s in spot)
    # print(f"DEBUG KuCoin futures data: {fut}")
    return {
        "futures": fut,
        "spot": spot,
        "total_spot_balance": total_spot,
        "total_equity": fut["accountequity"] + total_spot
    }

def fetch_positions_kucoin():
    try:
        endpoint = "/api/v1/positions"
        method = "GET"
        headers = _kucoin_headers(KUCOIN_API_KEY, KUCOIN_API_SECRET, KUCOIN_API_PASSPHRASE, method, endpoint)
        url = f"{KUCOIN_BASE_URL}{endpoint}"

        r = requests.get(url, headers=headers, timeout=20)
        r.raise_for_status()
        data = r.json()
        print(f"🔍 DEBUG KuCoin RAW positions response: {data}")
        
        rows = data.get("data", [])
        if not rows:
            print("🔍 DEBUG KuCoin: No positions data found")
            return []
        ...
        for pos in rows:
            ...
            unrealized_pnl = float(pos.get("unrealisedPnl", pos.get("unrealisedPNL", 0)))
            cumulative_trade_fee = float(pos.get("cumulativeTradeFee", 0))
            cumulative_funding_fee = float(pos.get("cumulativeFundingFee", 0))
            realised_pnl = float(pos.get("realisedPnl", 0))
            fee = -abs(float(pos.get("dealComm", 0)))  # Tomar valor absoluto
            funding_fee = float(pos.get("fundingFee", 0))
            ...
            positions.append({
                ...
                "unrealized_pnl": unrealized_pnl,
                "leverage": float(pos.get("leverage", 0)),
                "notional": float(pos.get("positionValue", pos.get("posCost", 0))),
        ...
        # print(f"✅ KuCoin positions found: {len(positions)}")
        return positions
...
def fetch_funding_kucoin(limit=100,symbol=None, startAt=None, endAt=None):
    try:
        ...
        if startAt:
            params.append(f"startAt={startAt}")
        if endAt:
            params.append(f"endAt={endAt}")

        query = "?" + "&".join(params) if params else ""
        full_endpoint = endpoint + query

        headers = _kucoin_headers(KUCOIN_API_KEY, KUCOIN_API_SECRET, KUCOIN_API_PASSPHRASE, "GET", full_endpoint)
        url = f"{KUCOIN_BASE_URL}{full_endpoint}"
        ...
        for f in rows:
            funding.append({
                "exchange": "kucoin",
                "symbol": f.get("symbol", "").replace("-", ""),
                "income": float(f.get("fundingFee", 0)),
...
def fetch_closed_positions_kucoin(symbol=None, start_time=None, end_time=None, limit=100, page_id=1):
    try:
        endpoint = "/api/v1/history-positions"
        params = []
        if symbol:
            params.append(f"symbol={symbol}")
        # validaciones: solo agregar from/to si no son None
        if start_time is not None:
            params.append(f"from={int(start_time)}")
        if end_time is not None:
            params.append(f"to={int(end_time)}")
        # limit máximo 200
        if limit is not None:
            limit_val = min(limit, 200)
            params.append(f"limit={limit_val}")
        if page_id is not None:
            params.append(f"pageId={page_id}")

        query = "?" + "&".join(params) if params else ""
        full_endpoint = endpoint + query

        print("DEBUG full_endpoint:", full_endpoint)
        ...
        url = KUCOIN_BASE_URL + full_endpoint
        r = requests.get(url, headers=headers, timeout=20)
        print("DEBUG status, resp:", r.status_code, r.text)
        r.raise_for_status()
        ...
        for pos in items:
            try:
                # DEBUG: Verificar campos reales
                # print(f"🔍 KuCoin closed position raw: {pos}")
                
                # Calcular size basado en realisedGrossCost y openPrice
                realised_gross_cost = float(pos.get("realisedGrossCost", 0))
                open_price = float(pos.get("openPrice", 1))
                size = abs(realised_gross_cost / open_price) if open_price != 0 else 0
                
                # Obtener valores según la documentación
                pnl_bruto = float(pos.get("realisedGrossCost", 0))  # PnL bruto (sin fees/funding)
                trade_fee = float(pos.get("tradeFee", 0))
                funding_fee = float(pos.get("fundingFee", 0))
                pnl_neto = float(pos.get("pnl", 0))  # PnL neto (después de fees/funding)
                ...
                results.append({
                    ...
                    "entry_price": float(pos.get("openPrice", 0)),
                    "close_price": float(pos.get("closePrice", 0)),
                    "realized_pnl": pnl_bruto,  # PnL BRUTO para que el frontend calcule correctamente
                    "funding_fee": funding_fee,
                    "fees": trade_fee,
                    "open_time": int(pos.get("openTime", 0) / 1000),
                    "close_time": int(pos.get("closeTime", 0) / 1000),
                    "leverage": float(pos.get("leverage", 0)),
                    "margin_mode": pos.get("marginMode", ""),
                    "size": size,
                })
                
                # print(f"🔍 KuCoin processed - PnL bruto: {pnl_bruto}, Fees: {trade_fee}, Funding: {funding_fee}, Size: {size}")
                
            except Exception as e:
                print(f"⚠️ Error parsing closed position: {e}, pos={pos}")
        return results
...
def save_kucoin_closed_positions(db_path="portfolio.db", days=90, debug=False):
    """
    Guarda las posiciones cerradas de KuCoin (últimos 3 meses) en la base de datos SQLite.
    """
    import time
    now = int(time.time() * 1000)
    start = now - days * 24 * 60 * 60 * 1000
    positions = fetch_closed_positions_kucoin(start_time=start, end_time=now)

    if not positions:
        print("⚠️ No se obtuvieron posiciones cerradas de KuCoin.")
        return
    ...
    conn.close()
    print(f"✅ KuCoin guardadas: {saved} | omitidas (duplicadas): {skipped}")
...
def main_balances():
    ...
    balance_functions = {
        ...
        "bingx":    lambda: fetch_account_bingx()(account_types=("spot", "perp")),  # ejemplo
        "aster":    lambda: fetch_account_aster()(db_path="portfolio.db", include_subaccounts=False),
        ...
        "kucoin":   lambda: fetch_account_kucoin_futures(db_path="portfolio.db", debug=False),
    ...
    try:
        ...
        for base, items in by_base.items():
            ...
            for c in clusters:
                ...
                # Calcular PnL TOTAL como el frontend lo espera: (close - entry) * size para cada posición
                pnl_total = 0
                for x in legs:
...
def get_positions():
    ...
    try:
     ...
     #binance positions  
    try:
        binance_positions = fetch_positions_binance_enriched()
   ...
   # BingX (nuevo 🚀) → fuera del bloque de Aster
    ...
    try:
        kucoin_positions = fetch_positions_kucoin()
        all_positions.extend(kucoin_positions)
        print(f"✅ KuCoin posiciones: {len(kucoin_positions)}")
        # DEBUG adicional para KuCoin
        for pos in kucoin_positions:
            print(f"🔍 KuCoin final position - Symbol: {pos['symbol']}, Unrealized PnL: {pos.get('unrealized_pnl', 'MISSING')}")
    except Exception as e:
        print(f"❌ Aden positions error: {e}")
    ...
    try:
        gate_positions = fetch_gate_open_positions(settle="usdt")
        all_positions.extend(gate_positions)
        print(f"✅ Gate.io posiciones: {len(gate_positions)}")
    except Exception as e:
...
def get_funding():
    ...
    # Extended 
    extended_funding = fetch_funding_extended(limit=50, debug=False)
    all_funding.extend(extended_funding)
    #print(f"✅ Extended funding: {len(extended_funding)} registros")

    #Kucoin
    kucoin_funding = fetch_funding_kucoin(limit=50)
    all_funding.extend(kucoin_funding)
    #print(f"✅ Kucoin funding: {len(extended_funding)} registros")
    
    try:
        gate_funding = fetch_gate_funding_fees(settle="usdt", limit=50)
        all_funding.extend(gate_funding)
        print(f"✅ Gate.io funding: {len(gate_funding)} registros")
    except Exception as e:
        print(f"❌ Gate.io funding error: {e}")
...
def get_balances():
    ...
    # KuCoin
    ku_all = fetch_all_balances_kucoin()
    if ku_all:
        ku_obj = {
            "exchange": "kucoin",
            "equity": ku_all["total_equity"],
            "balance": ku_all["futures"].get("balance", 0),  # Cambiado de availableBalance a balance
            "unrealized_pnl": ku_all["futures"].get("unrealized_pnl", 0),
            "spot": ku_all["total_spot_balance"],
            "margin": 0,  # KuCoin no tiene margin en este contexto
            "futures": ku_all["futures"].get("accountequity", 0),  # Usar accountequity en minúscula
        }
        balances.append(ku_obj)
        #print(f"✅ Kucoin futures balance: {ku_obj['futures']}")
        #print(f"✅ Kucoin total equity: {ku_obj['equity']}")
...
def main():
    ...
    # Diccionario con las funciones de sincronización de cada exchange
    sync_functions = {
        "backpack": lambda: save_backpack_closed_positions("portfolio.db"),
        "aden": lambda: save_aden_closed_positions("portfolio.db", debug=False),
        "bingx": lambda: sync_bingx(),
        "aster": lambda: save_aster_closed_positions("portfolio.db", days=30, debug=False),
        "binance": lambda: save_binance_closed_positions("portfolio.db", days=30, debug=False),
        "extended": lambda: save_extended_closed_positions("portfolio.db", debug=False),
        "kucoin": lambda: save_kucoin_closed_positions("portfolio.db", debug=False),
        "gate": lambda: save_gate_closed_positions("portfolio.db")
    }
