...
def fetch_closed_positions_aster(days=7, limit=1000, debug=False):
    """
    ...
    # --- 2️⃣ Descargar trades en bloques de 7 días ---
    for sym in symbols:
        trades = []
        cursor = start
        ...
        while cursor < now:
            ...
            try:
                ...
                if items:
                    trades.extend(items)
                    if debug:
                        print(f"[Aster] {sym}: {len(items)} trades ({cursor:%Y-%m-%d} → {end:%Y-%m-%d})")
            except Exception as e:
                if debug:
                    print(f"[Aster] Error fetching {sym}: {e}")
            cursor += step
