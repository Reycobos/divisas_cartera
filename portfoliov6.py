...
PRINT_OPEN_POSITIONS = True     # 3) Posiciones abiertas (resumen por exchange + bloques por símbolo)
PRINT_FUNDING = True            # 4) Funding: solo # de registros por exchange
PRINT_BALANCES = True           # 5) Balances: solo equity total por exchange
...
#     fetch_binance_all_balances,
#     fetch_binance_open_positions, 
#     fetch_binance_funding_fees,
#     save_binance_closed_positions
# )
...
MEXC_API_KEY = os.getenv("MEXC_API_KEY") or os.getenv("MEXC_ACCESS_KEY")
MEXC_API_SECRET = os.getenv("MEXC_API_SECRET") or os.getenv("MEXC_SECRET_KEY")
...
def main_balances():
    ...
    balance_functions = {
        ...
        "bitget": lambda: fetch_bitget_all_balances(),
    }
    # ---------------------------------------------------------------
    ...
    try:
        ...
        cur.execute("""
             SELECT exchange, symbol, side, size, entry_price, close_price, pnl,
                    ...
                    fee_total AS fees, pnl_percent, apr, initial_margin, notional, open_time, close_time
        ...
        for base, items in by_base.items():
            ...
            for c in clusters:
                ...
                pnl_price_sum = sum(float(x.get("pnl") or 0.0) for x in legs) #nuevo esquema
                ...
                groups.append({
                    ...
                    "pnl_total": pnl_total,   
                    "pnl_price_sum": pnl_price_sum, #nuevo esquema
                    "pnl_price_avg":(sum(float(x.get("pnl_percent") or 0.0) for x in legs) / max(len(legs), 1)),#nuevo esquema
                    "apr_avg":(sum(float(x.get("apr") or 0.0) for x in legs) / max(len(legs), 1)),#nuevo esquema
...
#     try:
#         bingx_positions = fetch_bingx_open_positions()
...
#     try:
#         kucoin_positions = fetch_kucoin_open_positions()
...
#         print(f"✅ Mexc posiciones: {len(gate_positions)}")
#     except Exception as e:
...
def get_funding():
    ...
    print(f"✅ Kucoin funding: {len(kucoin_funding)} registros")
    
    # Mexc
    mexc_funding = fetch_mexc_funding_fees()
    all_funding.extend(mexc_funding)
    print(f"✅ Mexc funding: {len(mexc_funding)} registros")
    

    
    # Gate
    gate_funding = fetch_gate_funding_fees(limit=50)
    all_funding.extend(gate_funding)
    print(f"✅ Gate funding: {len(gate_funding)} registros")
    ...
    #Bitget
    try:
        bitget_funding = fetch_bitget_funding_fees(limit=50)
        all_funding.extend(bitget_funding)
        print(f"✅ Bitget funding: {len(bitget_funding)} registros")
    except Exception as e:
...
def get_positions():
    ...
    # Aster
    try:
        all_positions.extend(fetch_positions_aster())
    except Exception as e:
        print(f"❌ Aster positions error: {e}")
...
def main():
    ...
    sync_functions = {
        ...
        "bingx": lambda: save_bingx_closed_positions("portfolio.db", symbols=["BTCUSDT","ETHUSDT"], days=30),
        "mexc": lambda: save_mexc_closed_positions("portfolio.db"),
        "bitget": lambda: save_bitget_closed_positions("portfolio.db", days=30, debug=False),
    }
