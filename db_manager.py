...
def init_db():
    ...
    _add_col_if_missing(conn, "closed_positions", "ignore_trade", "INTEGER DEFAULT 0")  # ← NUEVO
    conn.commit()
    conn.close()
...
def migrate_spot_support():
    """Migración para agregar columna ignore_trade"""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    try:
        cur.execute("ALTER TABLE closed_positions ADD COLUMN ignore_trade BOOLEAN DEFAULT 0")
        print("✅ Columna ignore_trade agregada a closed_positions")
    except sqlite3.OperationalError:
        print("ℹ️  Columna ignore_trade ya existe")
...
def upsert_funding_events(events: list, db_path=DB_PATH) -> int:
    ...
    for e in events:
        ...
        cur.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
        """, (exchange, symbol, asset, income, frate, phours, ts_ms, ext_id, typ, est, ext_hash, raw))
        if cur.rowcount > 0:
            inserted += 1
    conn.commit()
    conn.close()
...
def last_funding_ts(exchange: str, db_path=DB_PATH) -> int:
    ...
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("SELECT COALESCE(MAX(timestamp),0) FROM funding_events WHERE exchange = ?", (exchange,))
    val = cur.fetchone()[0] or 0
    conn.close()
    return int(val)

def load_funding(days: int = 7, exchange: str = None, symbol: str = None,
                 include_estimates: bool = True, limit: int = 5000, db_path=DB_PATH) -> list:
    """Lee eventos de funding desde DB, opcionalmente filtrando."""
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    conds = []
    args  = []
    if days and days > 0:
        ...
        conds.append("timestamp >= ?"); args.append(since_ms)
    if exchange:
        conds.append("exchange = ?"); args.append(exchange)
    if symbol:
